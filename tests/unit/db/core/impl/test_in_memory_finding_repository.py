...
class TestInMemoryFindingRepository:
    ...
    @pytest.mark.asyncio
    async def test_store_and_get_findings(self):
        ...
        await repo.store_finding(finding1)
        await repo.store_finding(finding2)

        findings = await repo.get_findings(doc_id)
    ...
    @pytest.mark.asyncio
    async def test_get_findings_for_nonexistent_document(self):
    ...
    @pytest.mark.asyncio
    async def test_get_findings_sorted_by_confidence(self):
        ...
        await repo.store_finding(finding1)
        await repo.store_finding(finding2)
        await repo.store_finding(finding3)
    ...
    @pytest.mark.asyncio
    async def test_get_findings_multiple_documents(self):
        ...
        findings_doc1 = await repo.get_findings(doc_id1)
        findings_doc2 = await repo.get_findings(doc_id2)

        assert len(findings_doc1) == 2
        assert len(findings_doc2) == 1

    @pytest.mark.asyncio
    async def test_get_all_findings_empty(self):
        """Test getting all findings when repository is empty."""
        repo = InMemoryFindingRepository()
        findings = await repo.get_all_findings()
        assert findings == []

    @pytest.mark.asyncio
    async def test_get_all_findings(self):
    ...
    @pytest.mark.asyncio
    async def test_get_all_findings_with_type_filter(self):
        ...
        ssn_findings = await repo.get_all_findings(finding_type=FindingType.SSN)
        email_findings = await repo.get_all_findings(finding_type=FindingType.EMAIL)
    ...
    @pytest.mark.asyncio
    async def test_get_all_findings_with_pagination(self):
        ...
        for i in range(5):
            ...
            await repo.store_finding(finding)

        # Get first 2
        page1 = await repo.get_all_findings(limit=2, offset=0)
        assert len(page1) == 2

        # Get next 2
        page2 = await repo.get_all_findings(limit=2, offset=2)
        assert len(page2) == 2

        # Get last page
        page3 = await repo.get_all_findings(limit=2, offset=4)
        assert len(page3) == 1

    @pytest.mark.asyncio
    async def test_count_findings_empty(self):
        """Test counting findings when repository is empty."""
        repo = InMemoryFindingRepository()
        count = await repo.count_findings()
        assert count == 0

    @pytest.mark.asyncio
    async def test_count_findings_total(self):
    ...
    @pytest.mark.asyncio
    async def test_count_findings_by_document(self):
        ...
        count_doc1 = await repo.count_findings(document_id=doc_id1)
        count_doc2 = await repo.count_findings(document_id=doc_id2)
        count_total = await repo.count_findings()
    ...
    @pytest.mark.asyncio
    async def test_count_findings_for_nonexistent_document(self):
        """Test counting findings for a document with no findings."""
        repo = InMemoryFindingRepository()
        doc_id = UUID("123e4567-e89b-12d3-a456-426614174000")

        count = await repo.count_findings(document_id=doc_id)
        assert count == 0
